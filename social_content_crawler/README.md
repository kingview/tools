...
Telegram Web 下载最新图文/视频：先调用 `social.browse_posts`，使用 `source="url"`、
`view="latest"`、频道 `start_url` 和 `max_items=1`，再将返回的消息 URL 交给
`social.download_media`（`telegram_scope="messages"`）。浏览通过 Telegram 原生向下箭头
跳到频道底部，等待按钮消失及消息稳定，再按消息编号倒序返回；不能仅用滚动高度稳定认定最新。
`latest` 不跳过最底部的纯文本消息；仅明确允许筛选媒体时使用 `view="media"`。单条定位最多
20 秒，连续三次滚动没有变化即停止并记录错误，不会为了找一条消息遍历全部历史。

Telegram 登录校验区分“标签未恢复”“页面加载中”和“明确显示登录表单”。等待最多 45 秒；
恢复的标签若持续 5 秒仍没有 body，仅对该空白页面自动刷新一次，不刷新已加载的聊天或登录表单。
加载失败会提示检查页面/网络，不要求重新登录或注册；比特浏览器本地 API 始终绕过系统代理。

Agent 下载过程按 execution_id 输出 `transfer-progress/<execution_id>.json`，并把下载状态写到
`logs/transfer-<execution_id>.jsonl`；只记录文件名、字节数、速度和帖子序号，不记录帖子正文或认证数据。
Telegram 分片读取使用 45 秒超时，任务授权撤销后停止读取后续分片。失败保留完成文件及 `.part`，
相同请求重试复用目录，并在核对远端大小和分片范围后续传。Agent 总步骤进度与文件下载进度分开显示。
...
3. 推荐把窗口命名为 `平台-用途-编号`，例如 `DY-素材-01`、`XHS-美女-01`、`X-发布-01`、`TG-频道-01`。也支持抖音、小红书、Twitter、Telegram 等中英文平台前缀。名称必须以平台前缀开头，防止把普通窗口误注册。
4. 在 Social Agent 或 PostDrop 中点击“管理浏览器窗口”。填写本地 API 地址并点击“自动发现并注册”，PostDrop 会扫描全部 Profile，按名称判断平台，验证登录状态并批量注册；已经注册的窗口会保留原 `session_ref`，名称不符合规则的窗口会跳过。
5. 不使用命名规则时，可以点击“注册新窗口”继续手动注册。
6. 手动注册时，从比特浏览器“系统设置”复制本地 API 地址，点击“读取 Profile”，选择相同平台和已登录 Profile，再点击“注册并完成”。成功保存后注册弹框才关闭，管理列表立即刷新并选中新窗口。也可先点击“生成 session_ref”复制引用，再点“完成”（不会重复生成）；更换平台、API 地址或 Profile 后必须重新注册。“取消”仅关闭弹框，不注册新会话。注册页不包含历史窗口管理入口；移除已有引用请在管理窗口列表中选择后点击“移除引用”，不会删除比特浏览器 Profile 或退出账号。关闭管理窗口后，Agent 主界面同步刷新可用会话。
